...
public class VillagerAStarNew
{
	...
	private void RefreshOccupiedCells(BlockPos start)
	{
		...
		// CHANGED FROM UPSTREAM: was a check on VS Village's own Village.IsGatherActive.
		// Replaced with an interface this mod owns, so the pathfinder has no knowledge of
		// villages at all - anything that wants crowd avoidance switched off says so.
		if ((owner as IPathCrowdPolicy)?.IgnoreCrowding == true)
		{
			occupiedCells = null;
			return;
		}
		...
		Vintagestory.API.Common.Entities.Entity[] nearby = world.GetEntitiesAround(center, 50f, 10f,
			// CHANGED FROM UPSTREAM: was HasBehavior<EntityBehaviorVillager>().
			e => e != owner
				&& (e is EntityPlayer || e is FFVillager));
